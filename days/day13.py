...
def Answer(aList):
    ...
    # DO IT
    # part 1
    timeStamp = int(aList[0].strip())
    buses = [int(bus) for bus in list(filter(('x').__ne__, aList[1].strip().split(',')))]
    busTime = [0, 1000]
    for bus in buses:
        if (math.ceil(timeStamp / bus) * bus - timeStamp) < busTime[1]:
            busTime = [bus, (math.ceil(timeStamp / bus) * bus - timeStamp)] 
    part1 = busTime[0] * busTime[1]

    # part 2
    busMinPairs = [[int(i), int(j)] for j, i in enumerate(aList[1].strip().split(',')) if i != 'x']

    # iterate through buses
    time, step = 0, 1
    for bus, mins in busMinPairs:
        # check to see if bus is departing at current time
        while (time + mins) % bus != 0:
            time += step
        step *= bus
    part2 = time
    #
