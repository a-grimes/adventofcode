...
def Answer(aList):
    ...
    aList.append(0) # seat outlet
    aList.append(max(aList)+3) # device @ +3 jolts
    aList.sort()
    i = 1
    expCount = 0
    while i < len(aList):
        if aList[i] - aList[i-1] == 1:
            diffDict["1"] += 1
        elif aList[i] - aList[i-1] == 2:
            diffDict["2"] += 1
        elif aList[i] - aList[i-1] == 3:
            diffDict["3"] += 1
        i += 1
    
    part1 = diffDict["1"] * diffDict["3"]

    bList = [1]+[0]*aList[-1]
    for i in aList[1:]:
        bList[i] = bList[i-3] + bList[i-2] + bList[i-1]
    part2 = bList[-1]
    #
