...
def Answer(aList):
    ...
    modReqFields = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    regexBYR = "^byr((19[2-9]\d{1})|(200[0-2]))$"
    regexIYR = "^iyr20((1[0-9])|(20))$"
    regexEYR = "^eyr20((2[0-9])|(30))$"
    regexHGT = "^hgt((1(([5-8][0-9])|(9[0-3]))cm)|(((59)|(6[0-9])|(7[0-6]))in))$"
    regexHCL = "^hcl#[0-9a-f]{6}$"
    regexECL = "^ecl((amb)|(blu)|(brn)|(gry)|(grn)|(hzl)|(oth))$"
    regexPID = "^pid[0-9]{9}$"
    regex = "({})|({})|({})|({})|({})|({})|({})".format(regexBYR, regexIYR, regexEYR, regexHGT, regexHCL, regexECL, regexPID)
    passportDict = {}
    ...
    for line in aList:
        if not line: # new passport
            count1 = 0
            count2 = 0
            for key in passportDict.keys():
                if key in modReqFields:
                    count1 += 1
                if re.search(regex, "{}{}".format(key, passportDict[key])):
                    count2 += 1
            if count1 >= len(modReqFields):
                part1 += 1
            if count2 >= len(modReqFields):
                part2 += 1
            passportDict = {}
        else:
            fields = line.split()
