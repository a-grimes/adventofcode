...
def Answer(aList):
    ...
    currIndex = preamble
    while currIndex < len(aList):
        passed = 0
        for i in range(currIndex - preamble, currIndex): # range in non-inclusive of END
            for j in range(currIndex - preamble, currIndex): # range in non-inclusive of END
                if i == j:
                    continue
                if aList[i] + aList[j] == aList[currIndex]:
                    passed = 1
        if not passed:
            part1 = aList[currIndex]
            break
        currIndex += 1
